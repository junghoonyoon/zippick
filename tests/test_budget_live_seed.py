...
class BudgetLiveSeedTest(unittest.TestCase):
    def test_fast_mode_skips_live_seed_and_signal_enrichment(self):
        with mock.patch.object(budget_candidates.molit_transactions, "enabled", return_value=True), \
        ...
        self.assertEqual(result["liveSeedCount"], 0)
    ...
    def test_rental_and_lh_complexes_are_excluded_before_live_price_lookup(self):
        ...
        with mock.patch.object(budget_candidates, "_load_price_bands", return_value=[]), \
             mock.patch.object(budget_candidates.real_estate_search, "APARTMENT_MASTER", entities), \
             mock.patch.object(budget_candidates.molit_transactions, "enabled", return_value=True), \
             mock.patch.object(budget_candidates.molit_transactions, "price_band_for_apartment", return_value=live) as lookup, \
             mock.patch.object(budget_candidates.molit_transactions, "last_error", return_value=""):
            budget_candidates._ENTITY_LOOKUP = None
            result = budget_candidates.budget_candidates(
    ...
    def test_old_last_deal_metadata_remains_a_reference_price(self):
        ...
        self.assertEqual(row["budgetGapEok"], 0)
    ...
    def test_all_matches_excludes_last_observed_deal_over_purchase_ceiling(self):
        entities = [
            {
                "name": name,
                "city": "성남시",
            ...
            for name in ("상한초과아파트", "상한안아파트")
        ]

        def last_deal(name, **_kwargs):
            return {
                "latestDealPriceEok": 12.1 if name == "상한초과아파트" else 11.8,
        ...
        self.assertEqual([row["name"] for row in result["candidates"]], ["상한안아파트"])
        self.assertEqual(result["candidates"][0]["priceRangeText"], "마지막 실거래 11억 8,000만원")
        self.assertEqual(result["candidates"][0]["lastObservedDealDate"], "2025-05-16")
    ...
    def test_all_matches_excludes_verified_price_far_above_budget(self):
        ...
        with mock.patch.object(budget_candidates, "_load_price_bands", return_value=[]), \
            ...
            budget_candidates._ENTITY_LOOKUP = None
            result = budget_candidates.budget_candidates(
                "8.9억", region="서울시", min_area="59", all_matches=True,
            )
